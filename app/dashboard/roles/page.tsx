            ...
            <div className="p-5 flex flex-col flex-1">
              ...
              <div className="flex flex-wrap gap-1 mb-5 flex-1">
                {role.permissions.slice(0, 4).map(rp => (
                  <span key={rp.id} className={`text-[10px] px-1.5 py-0.5 rounded-md border font-medium leading-tight ${SCOPE_COLORS[rp.scope]}`}>
                    {rp.permission.label}
                  </span>
                ))}
                {role.permissions.length > 4 && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-slate-100 text-slate-500 font-medium leading-tight">
              ...
              {canManage && (
                <div className="flex gap-2 pt-3 border-t border-slate-100 mt-auto">
                  ...
                  <Button variant="outline" size="sm" onClick={() => handleDelete(role.id)} className="!border-red-500 !text-red-500 hover:!bg-red-50">
                    <svg className="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
